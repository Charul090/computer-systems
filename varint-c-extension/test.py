...
if __name__ == '__main__':
    ...
    for fname, expectation in cases:
        with open(fname, 'rb') as f:
            ...
            ans243 = cvarint.decode(cvarint.encode(n))
            assert ans243 == n
    ...
    for _ in range(num_cases):
        ...
        # time the C
        start = time.process_time_ns()
        enc2 = cvarint.encode(n)
        res2 = cvarint.decode(enc2)
        end = time.process_time_ns()
        c_time += (end - start)
        try:
            assert n == res2
        except AssertionError:
            print(f'Failed on {n}: round trip result was encoded {enc}:{enc2} decoded {res}:{res2}')
            break
